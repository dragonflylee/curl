                    ...
                    struct Curl_easy *data)
{
    ...
    { /* Only setup the context if the application didn't. */
        /* Load the trusted CA */
        if (ssl_cafile)
        {
                                    ...
                                    struct Curl_easy *data)
{
    ...
    if (cf->conn->bits.tls_enable_alpn)
    {
        ...
        Curl_multiuse_state(cf->conn, cf->conn->alpn == CURL_HTTP_VERSION_2 ? BUNDLE_MULTIPLEX : BUNDLE_NO_MULTIUSE);
    }
#endif
...
static CURLcode libnx_send(struct Curl_cfilter *cf, struct Curl_easy *data,
                          ...
                          size_t *pnwritten)
{
    ...
    *pnwritten = 0;
    ...
    rc = sslConnectionWrite(&BACKEND->conn, mem, len, (u32 *)pnwritten);

    if (R_FAILED(rc))
    {
        /* PR_WOULD_BLOCK_ERROR */
        return (R_VALUE(rc) == MAKERESULT(123, 204)) ? CURLE_AGAIN : CURLE_WRITE_ERROR;
    }
...
static CURLcode libnx_recv(struct Curl_cfilter *cf, struct Curl_easy *data,
                          ...
                          size_t *pnread)
{
    ...
    *pnread = 0;
    ...
    rc = sslConnectionRead(&BACKEND->conn, buf, buffersize, (u32 *)pnread);

    if (R_FAILED(rc))
    {
        /* PR_WOULD_BLOCK_ERROR */
        return (R_VALUE(rc) == MAKERESULT(123, 204)) ? CURLE_AGAIN : CURLE_RECV_ERROR;
    }
                                   ...
                                   bool *done)
{
    ...
    *done = FALSE;
    ...
    if (ssl_connect_1 == connssl->connecting_state)
    {
        ...
        retcode = libnx_connect_step1(cf, data);
    }
